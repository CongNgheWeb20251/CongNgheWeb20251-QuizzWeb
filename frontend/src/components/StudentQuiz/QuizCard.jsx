    ...
    >
      ...
      <div className="px-6 py-4 border-t border-gray-100 bg-gray-100 mt-auto">
        <div className="flex gap-2">
          {
            // 1) Có session và session đã done → Results + Retake (nếu còn hạn và cho phép)
            quiz.sessionsCount > 0 && lastSession?.status === 'completed' ? (
              <>
                <button
                  className={`${!isExpired && quiz.allowRetake ? 'flex-1' : 'w-full'} px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition-all flex items-center justify-center gap-2`}
